...
// Axios interceptor to log API errors for debugging
tmdb.interceptors.response.use(
  (response) => response,
  (error) => {
    if (error.response) {
      console.error(`MovieBase API Error ${error.response.status}:`, error.response.data?.status_message || error.message);
    } else if (error.request) {
      console.error("MovieBase API Network Error: No response received. Check your internet connection.");
    } else {
      console.error("MovieBase API Error:", error.message);
    }
...
function transformReview(review) {
  return {
    ...
    rating: review.author_details?.rating || 0, // 0-10 scale
...
export const createReview = () => Promise.reject(new Error("Reviews are read-only in this experience"));
